...
<?php if ( have_comments() ) : ?><!--you need the id comments for the links to the comments-->
    	<h4 class="comments-header-title"><?php comments_number(__('No Comments','smooththemes'), __('One Comment','smooththemes'), __('% Comments','smooththemes') );?></h4>
...
<?php else : ?>
	...
	'fields' => apply_filters( 'comment_form_default_fields', array(	
			'a_before'=>'<div class="row">',
    		'author' => '<div class="four columns b0"> <div class="form-line comment-form-author">' . '<label for="author">' . __( 'Name','smooththemes' ) . '</label> ' . ( $req ? '<span class="required">*</span>' : '' ) . '<br /><input id="author" name="author" type="text" value="' . esc_attr( $comment_author) . '" size="30"' . $aria_req . ' /></div> </div>',
    		'email' => '<div class="four columns b0"> <div class="form-line comment-form-email"><label for="email">' . __( 'Email','smooththemes' ) . '</label> ' . ( $req ? '<span class="required">*</span>' : '' ) . '<br /><input id="email" name="email" type="text" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . ' /></div> </div>',
    		'url' => '<div class="four columns b0"> <div class="form-line comment-form-url"><label for="url">' . __( 'Website','smooththemes' ) . '</label>' . '<br /><input id="url" name="url" type="text" value="' . esc_attr($comment_author_url ) . '" size="30" /></div> </div>',
			'z_after'=>'</div>',
			) )
		 );
