	...
	while ( $query->have_posts() ) : $query->the_post();
            ...
            <li  id-name="imageid_<?php echo $post->ID; ?>" img-id="<?php echo $post->ID; ?>" class="add_img">
                <div class="imw stpb-hndle">
                     <img  class="imgid" src="<?php echo $attachment[0]; ?>"  title="<?php echo esc_attr(the_title('','',false)); ?>"  />
                     ...
                     <input type="hidden" class="btnlabel" value="" />
...
function stpb_get_pages(){
	...
	foreach( $pages as $page ) {
		...
		$st_page_options =  get_page_builder_options($page->ID);
		if(!isset($st_page_options) || !isset($st_page_options['tagline'])){
			$st_page_options['tagline']='';
		}
		
		$content = apply_filters( 'the_content', $content );
		?>
		<div  class="item item-id-<?php echo esc_attr($page->ID); ?>">
			<span class="st-ptt"><a target="_blank"  href="<?php echo get_page_link( $page->ID ); ?>"><?php echo esc_html($page->post_title); ?></a></span>
			<a href="#" class="st-add" data-title="<?php echo esc_attr($page->post_title); ?>" data-tagline="<?php echo esc_attr($st_page_options['tagline']); ?>" data-id="<?php echo esc_attr($page->ID); ?>"><?php _e('Add','smooththemes'); ?></a>
			<div class="clear"></div>
		</div>
		<?php
	}
...
add_action('wp_ajax_stpb_get_pages', 'stpb_get_pages');
