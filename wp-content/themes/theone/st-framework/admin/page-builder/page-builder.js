function create_font_awesome_icons(){
	if(typeof(window.font_awesome_icons)!=='undefined'){
		//alert('ok');
 //console.debug(font_awesome_icons);
		var i =0;
		for (var k in font_awesome_icons) {
		  
		  if (window.font_awesome_icons.hasOwnProperty(k)) { 
		  // or if (Object.prototype.hasOwnProperty.call(obj,prop)) for safety...
		   // alert("prop: " + prop + " value: " + obj[prop])
			  jQuery('.st-list-icons').append('<i class="'+k+'"></i>');
			  i++;
		  }
		}

		
		jQuery('.st-select-icon i').live('click',function(){
			var  p  = jQuery(this).parents('.st-select-icon');
			var n = jQuery(this).attr('class');
			jQuery('.st-icon-value',p).val(n);
			jQuery('.st-icon-preview',p).html('<i class="'+n+'"></i>');;
			//jQuery('.preview-wrapper',p).show();
				
		});
		
		jQuery('.st-select-icon .remove-icon').live('click',function(){
			var  p  = jQuery(this).parents('.st-select-icon');
			jQuery('.st-icon-value',p).val('');
			jQuery('.st-icon-preview',p).html('');
			// jQuery('.preview-wrapper',p).hide();
				
			return false;
		});
		
		jQuery('.st-select-icon .st-icon-value').live('keyup',function(){
			var  p  = jQuery(this).parents('.st-select-icon');
			var n = jQuery(this).val();
			if(jQuery('.st-list-icons i.'+n).length>0){
	...
	}// end check typeof(font_awesome_icons)
...
function re_name_gallery_meta(g) {
    ...
    jQuery('.stpb-img-items li', g).each(function (i) {
        ...
        jQuery('.btnlabel', li).attr('name', pre_name + '[' + i + '][btnlabel]');
    });
}
...
jQuery(document).ready(function () {
    ...
    jQuery('.stpb-gallery  li .stpb_edit').live('click', function () {
        ...
        jQuery('.stpbn-btnlabel', box).val(jQuery('.btnlabel', li).val());
    ...
    jQuery('.stpb-gallery .g-save-meta').live('click', function () {
        ...
        jQuery('.gurl', li).val(jQuery('.stpbn-url', box).val());
        jQuery('.btnlabel', li).val(jQuery('.stpbn-btnlabel', box).val());

        // remove data
        jQuery('.stpbn-title', box).val('');
        jQuery('.stpbn-caption', box).val('');
        jQuery('.stpbn-url', box).val('');
        jQuery('.stpbn-btnlabel', box).val('');
    ...
    // for home elements
    jQuery('.stpb_pd_w .home-element-wrapper .tt').live('change', function () {
        var gp = jQuery(this).parents('.home-element-wrapper');
        if (jQuery(this).val() == 'fixed-height-slider') {
            jQuery('.layer-slider, .fixed-video', gp).hide(200, function () {
                jQuery('.fixed-height-slider', gp).show();
            });

        } else if (jQuery(this).val() == 'layer-slider') {
            jQuery('.fixed-height-slider, .fixed-video', gp).hide(200, function () {
                jQuery('.layer-slider', gp).show();
            });
        } else if (jQuery(this).val() == 'fixed-height-video') {
            jQuery('.layer-slider, .fixed-height-slider', gp).hide(function () {
                jQuery('.fixed-video', gp).show();
            });
        }else {
            jQuery('.layer-slider, .fixed-video', gp).hide(200, function () {
                jQuery('.fixed-height-slider', gp).show();
    ...
    // For show page title and tag line - MichaleKing
    jQuery('.stpb_pd_w  .show_title_ibutton').iButton({
        change: function (t) {
            var p = t.parents('.stpb_pd_w');
            if (t.attr('checked')) {
                jQuery('.slider-show-title', p).show();
                // show slider type
                var st = jQuery('.st-slider-type', p).val();
                jQuery('.st-' + st, p).show();
                ...
                jQuery('.slider-show-title', p).hide();
            }
        }
    ...
    function page_template_changed(){
   	 if(jQuery('select#page_template').length){
   		 ...
   		 switch(file_name){
   		 	case 'tpl-one-page.php':
   		 		jQuery('.stpb_pd_w').hide();
   		 		jQuery('.st-one-page-settings').show();
   			 break;
   		  	...
   		  	default:
   		  		jQuery('.stpb_pd_w').hide();
 			   jQuery('.st-page-builder-inside').show();
   		  		 
   		 }
   		 
   		 jQuery('.stpb_pd_w_always_show').show();
