...
function st_get_video($url,&$return=array()){
	$url_lower = strtolower($url);
		...
		if(strpos($url_lower,'youtube')){		
                ...
                return '<iframe src="http://www.youtube.com/embed/'.$id[1].'?wmode=transparent" ></iframe>';
              ...
              return '<iframe src="http://www.youtube.com/embed/'.$id[1].'?wmode=transparent&autohide=true&showinfo=false" ></iframe>';
            ...
            return '<iframe src="http://player.vimeo.com/video/'.$id[1].'?title=0&amp;byline=0&amp;portrait=0" ></iframe>';
		}
		 return '';
	}

function st_list_post_thumbnail($post_id='', $size ='st_small_thumb',$small_video_thumb = false, $lightbox = false ){
     ...
     $html ='';
            ...
            case 'video':
                ...
                if ( has_post_thumbnail($post_id) ) {
                        ...
                        if($lightbox==true){
                            ...
                            $html ='<a href="'.$full_image_url[0].'" class="st-image-lightbox">
                                        <img src="'.$thumb_image_url[0].'" alt="'.esc_attr($post_title).'">
                                    </a>';
                        }else{
            ...
            case 'slider':
                ...
                if(count($st_page_options['thumbnails']['images'])){
                    ...
                    $slider_data  = array_merge( array('slider_items'=> $st_page_options['thumbnails'] ), array('show_caption'=>'no','class'=>'blog-thumb-wrapper','size'=>$size,'lightbox'=>$lightbox));
...
function st_post_thumbnail($post_id='', $size ='st_small_thumb',$small_video_thumb = false, $lightbox = false ){
     $st_page_options = get_page_builder_options($post_id);
...
function st_portfolio_thumbnail($post_id='', $size ='st_medium',$rand_if_slider= false,$small_video_thumb = false){
     ...
     $html ='';
            ...
            default;
                 if ( has_post_thumbnail($post_id) ) {
                     ...
                     $html ='<img src="'.$thumb_image_url[0].'" alt="'.esc_html($ptitle).'">';    
...
function st_portfolio_thumbnail_one($post_id='', $size ='st_medium',&$src=''){
...
function st_get_user_desc($user_id = 0) {
